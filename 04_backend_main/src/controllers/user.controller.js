...
const generateAcceesAndRefreshToken = async (userId) => {
    try {
...
const registerUser = asyncHandler(async (req, res) => {
    ...
    const user = await User.create({
        ...
        email,
        password,
        username: username ? username.toLowerCase() : email.split("@")[0], // fallback
    })
...
const loginUser = asyncHandler(async (req, res) => {
    ...
    const { accessToken, refreshToken } = await generateAcceesAndRefreshToken(user._id)
...
const refreshAccessToken = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const { accessToken, newrefreshToken } = await generateAcceesAndRefreshToken(user._id)
        return res
            .status(200)
            .cookie("accessToken", accessToken, options)
